      ...
      {faqs.map((faq, i) => (
        <div key={i} className="border-b border-[#e4e9f2] py-6">
          <div
            ...
            onClick={() => toggle(i)}
          >
            <h4 className="text-[#0a0d53] font-inter font-bold text-[16px] transition-colors hover:text-[#06bae1] pr-4">{faq.q}</h4>
            {openIndex === i ? (
              <div className="w-[16px] h-[3px] bg-[#06bae1] shrink-0 rounded-full transition-all duration-300" />
            ) : (
              <span className="text-[#06bae1] text-[24px] font-bold shrink-0 leading-none transition-all duration-300">+</span>
            )}
          </div>
          <div
            className={`overflow-hidden transition-all duration-300 ease-in-out ${
              openIndex === i ? "max-h-[200px] opacity-100 mt-4" : "max-h-0 opacity-0"
            }`}
          >
            <p className="text-[#5b6472] font-inter text-[13px] leading-relaxed pr-8">{faq.a}</p>
          </div>
        </div>
      ))}
    </div>
